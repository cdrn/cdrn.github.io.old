...
async function getWallet(walletPath, config, debug, colors) {
    ...
    if (walletPath) {
        if (typeof walletPath !== 'string') {
            console.log(utils_1.parseColor(colors, 'The wallet must be specified.', 'red'));
            return;
        }
        try {
            ...
            console.log(utils_1.parseColor(colors, 'Invalid wallet path.', 'red'));
            if (debug)
                console.log(e);
            return;
        }
    ...
    if (!wallet) {
        if (walletEncr) {
            ...
            try {
                ...
                console.log(utils_1.parseColor(colors, 'Invalid password.', 'red'));
                if (debug)
                    console.log(e);
                return;
            }
    ...
    if (!wallet) {
        console.log(utils_1.parseColor(colors, 'Save a wallet with `arkb wallet-save file-path.json`.', 'red'));
        return;
    }
    return wallet;
}
