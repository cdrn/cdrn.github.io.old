...
class CliCommands {
    ...
    async cliTask(partialArgs) {
        ...
        if (useBundler) {
            ...
            try {
                ...
                console.log(utils_1.parseColor(colors, '[--use-bundler] Invalid url format', 'red'));
        ...
        if (partialArgs.argv.license) {
            ...
            if (!spdx_license_ids_1.default.includes(license)) {
                ...
                if (spdxCandidates.length > 0) {
                    console.log(utils_1.parseColor(colors, 'Did you mean?', 'yellow'));
                    spdxCandidates.slice(0, 5).map((cand) => console.log(utils_1.parseColor(colors, ` ${cand.item}`, 'blue')));
                }
                else {
                    console.log(utils_1.parseColor(colors, `A list of valid spdx identifiers can be found at https://spdx.org/licenses/`, 'yellow'));
                }
                process.exit(1);
            }
        }
    ...
    showHelp(commandObj, command, partialArgs) {
        ...
        const colors = partialArgs.argv.colors;
        console.log(utils_1.parseColor(colors, `\nExample usage of ${utils_1.parseColor(colors, command, 'green')}:\n`, 'bold'));
        for (const option of commandObj.options) {
            const usage = commandObj.usage && commandObj.usage.length > 0
                ...
                : '';
            console.log(`${utils_1.parseColor(colors, `${option.description}:`, 'blackBright')}
