...
class Deploy {
    ...
    async prepare(dir, files, index = 'index.html', tags = new tags_1.default(), toIpfs = false, license, useBundler, feeMultiplier, forceRedeploy = false, colors = true) {
        this.txs = [];
        ...
        await promise_pool_1.default.for(files.filter((f) => !f.includes('manifest.arkb')))
            ...
            const type = mime_1.default.getType(filePath) || 'application/octet-stream';
            ...
            newTags.addTag('User-Agent-Version', utils_1.getPackageVersion());
        ...
        if (isFile && this.duplicates.length) {
            console.log(utils_1.parseColor(colors, 'File already deployed:', 'red'));
            if (toIpfs) {
                const data = fs_1.default.readFileSync(files[0]);
                const cid = await this.ipfs.hash(data);
                console.log(`IPFS: ${utils_1.parseColor(colors, cid, 'cyan')}`);
            }
            console.log('Arweave: ' + utils_1.parseColor(colors, `${this.blockweave.config.url}/${this.duplicates[0].id}`, 'cyan'));
            return;
        }
        if (this.logs) {
    ...
    async deploy(isFile = false, useBundler, colors = true) {
        ...
        try {
            ...
            if (target && (await this.blockweave.wallets.jwkToAddress(this.wallet)) !== target) {
                ...
                if (target.length) {
                    ...
                    tx.addTag('App-Version', utils_1.getPackageVersion());
        ...
        await promise_pool_1.default.for(toDeploy)
            ...
            if (useBundler) {
                try {
                    ...
                    console.log(e);
                    console.log(utils_1.parseColor(colors, 'Failed to deploy data item: ' + txData.filePath, 'red'));
                }
            }
            ...
            if (!deployed) {
                try {
                    await promises_1.pipeline(fs_1.createReadStream(txData.filePath), 
                    // @ts-ignore
                    arweave_stream_tx_1.uploadTransactionAsync(txData.tx, this.blockweave));
                    deployed = true;
                }
                catch (e) {
                    if (this.debug) {
                        console.log(e);
                        console.log(utils_1.parseColor(colors, `Failed to upload ${txData.filePath} using uploadTransactionAsync, trying normal upload...`, 'red'));
                    }
                }
            }
            if (!deployed) {
                try {
                    ...
                    if (this.debug) {
                        console.log(e);
                        console.log(utils_1.parseColor(colors, `Failed to upload ${txData.filePath} using normal post!`, 'red'));
                    }
                }
            }
        });
        ...
        if (!isFile) {
            ...
            if (id) {
                ...
                const mPath = path_1.default.resolve(utils_1.getTempDir(), `${id}.manifest.json`);
                try {
    ...
    async buildTransaction(filePath, tags) {
        const tx = await promises_1.pipeline(fs_1.createReadStream(filePath), arweave_stream_tx_1.createTransactionAsync({}, this.arweave, this.wallet));
    ...
    async buildManifest(dir, index = null, tags, useBundler, feeMultiplier) {
        ...
        try {
            fs_1.default.writeFileSync(path_1.default.join(utils_1.getTempDir(), `${tx.id}.manifest.json`), JSON.stringify(data));
        }
        catch (e) {
            /* */
        }
