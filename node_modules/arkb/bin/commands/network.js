...
const command = {
    ...
    execute: async (args) => {
        ...
        try {
            ...
            console.log(utils_1.parseColor(colors, `Network Details for ${blockweave.config.url}\n`, 'green'));
            Object.keys(net).forEach((key) => {
                const value = net[key];
                console.log(`${utils_1.parseColor(colors, utils_1.snakeCaseToTitleCase(key), 'yellow')}: ${utils_1.parseColor(colors, isNaN(value) ? value : utils_1.numbersForHumans(value), 'cyan')}`);
            });
        }
        catch (err) {
            console.log(utils_1.parseColor(colors, `Unable to reach ${blockweave.config.url} - ${err.message}`, 'red'));
            if (console_1.debug)
                console.log(err);
        }
    },
