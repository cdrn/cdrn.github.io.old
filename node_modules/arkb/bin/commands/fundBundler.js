...
const command = {
    ...
    execute: async (args) => {
        ...
        if (!commandValues || !commandValues.length) {
            console.log(utils_1.parseColor(colors, 'You forgot to set the amount.', 'red'));
            return;
        }
        ...
        if (!useBundler) {
            console.log(utils_1.parseColor(colors, 'Please set bundler address', 'red'));
            return;
        }
        ...
        try {
            ...
            console.log(utils_1.parseColor(colors, 'Error getting bundler address, see more info with the --debug option.', 'red'));
            if (debug)
                console.log(e);
            process.exit(1);
        }
        ...
        try {
            ...
            console.log(utils_1.parseColor(colors, `Bundler funded with ${amount.toString()} AR, transaction ID: ${tx.id}`, 'cyan'));
        }
        catch (e) {
            console.log(utils_1.parseColor(colors, 'Error funding bundler address, see more info with the --debug option.', 'red'));
            if (debug)
                console.log(e);
        }
    },
