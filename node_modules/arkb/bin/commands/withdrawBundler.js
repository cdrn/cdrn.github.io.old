...
const command = {
    ...
    execute: async (args) => {
        ...
        try {
            ...
            if (!addy) {
                console.log(utils_1.parseColor(colors, 'Error withdrawing to wallet', 'red'));
                return;
            }
            // Success response
            console.log(`${utils_1.parseColor(colors, addy, 'cyan')} has been funded with ${utils_1.parseColor(colors, `AR ${amnt} from bundler.`)}`, 'yellow');
        }
        catch (e) {
            console.log(utils_1.parseColor(colors, 'Error withdrawing to wallet', 'red'));
            if (debug)
