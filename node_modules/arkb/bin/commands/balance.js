...
const command = {
    ...
    execute: async (args) => {
        ...
        const wallet = await wallet_1.getWallet(walletPath, config, debug, colors);
        if (!wallet) {
            console.log(utils_1.parseColor(colors, 'Please set a wallet or run with the --wallet option.', 'red'));
            return;
        }
        let addy;
        try {
            ...
            console.log(utils_1.parseColor(colors, 'Unable to decrypt wallet address.', 'red'));
            if (debug)
                console.log(e);
        }
        if (useBundler) {
            try {
                ...
                console.log(`${utils_1.parseColor(colors, addy, 'cyan')} has a bundler balance of ${utils_1.parseColor(colors, `AR ${blockweave.ar.winstonToAr(bal.toString(), { formatted: true, decimals: 12, trim: true })}`, 'yellow')}`);
            }
            catch (e) {
                console.log(utils_1.parseColor(colors, 'Unable to retrieve bundler balance.', 'red'));
                if (debug)
                    console.log(e);
            }
            return;
        ...
        try {
            ...
            console.log(`${utils_1.parseColor(colors, addy, 'cyan')} has a balance of ${utils_1.parseColor(colors, `AR ${blockweave.ar.winstonToAr(bal, { formatted: true, decimals: 12, trim: true })}`, 'yellow')}`);
        }
        catch (e) {
            console.log(utils_1.parseColor(colors, 'Unable to retrieve wallet balance', 'red'));
            if (debug)
                console.log(e);
        }
    },
