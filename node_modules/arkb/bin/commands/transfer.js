...
const command = {
    ...
    execute: async (args) => {
        ...
        try {
            ...
            if (!utils_1.isValidWalletAddress(target)) {
                console.log(utils_1.parseColor(colors, 'Invalid target wallet address', 'redBright'));
                return;
            }
            // Check if the amount is a positive number
            if (isNaN(amount) || amount <= 0) {
                console.log(utils_1.parseColor(colors, 'Invalid amount', 'redBright'));
                return;
            }
            ...
            if (+bal < amount) {
                console.log(utils_1.parseColor(colors, 'Insufficient balance', 'redBright'));
                return;
            }
            ...
            console.log(utils_1.parseColor(colors, `Transfer successful! Transaction ID: ${txid}`, 'greenBright'));
        }
        catch (error) {
            console.log(utils_1.parseColor(colors, 'Unable to send funds.', 'redBright'));
            if (debug)
                console.log(error);
        }
    },
