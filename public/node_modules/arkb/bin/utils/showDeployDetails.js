...
async function showDeployDetails(txs, wallet, isFile = false, dir, blockweave, useBundler, bundler, license, bundlerApi, bundled, colors) {
    ...
    for (let i = 0, j = txs.length; i < j; i++) {
        ...
        if (dataSize) {
            size = utils_1.bytesForHumans(+dataSize);
            totalSize += +dataSize;
        }
    ...
    if (bundled.tx) {
        ...
        new Line()
            ...
            .column(utils_1.bytesForHumans(+size), 15)
            .column(ifd ? '-' : ar, 17)
            .column('Bundle', 30)
            .column('-', 20)
            .fill()
    ...
    console.log('');
    console.log(utils_1.parseColor(colors, 'Summary', 'cyan'));
    if (license) {
    ...
    console.log(`Total size: ${utils_1.bytesForHumans(totalSize)}`);
    ...
    console.log('');
    console.log(utils_1.parseColor(colors, 'Wallet', 'cyan'));
