...
const command = {
    ...
    execute: async (args) => {
        const { argv, commandValues, walletPath, config, debug, blockweave, tags, ipfsPublish, license, useBundler, feeMultiplier, autoConfirm, bundle, bundler, colors, } = args;
        // Get the wallet
        let wallet = await wallet_1.getWallet(walletPath, config, debug, colors);
        if (useBundler && !wallet) {
            wallet = await blockweave.wallets.generate();
        }
        if (!wallet) {
            console.log(utils_1.parseColor(colors, 'Please save a wallet or run with the --wallet option.', 'red'));
            return;
        }
        if (useBundler && bundle) {
            console.log(utils_1.parseColor(colors, 'You can not use a bundler and locally bundle at the same time', 'red'));
            return;
        }
        const concurrency = argv.concurrency || 5;
        const forceRedeploy = argv.force;
        // Check and get the specified directory or file
        const dir = deploy_2.getDeployPath(commandValues, colors);
        let files = [dir];
        let isFile = true;
        if (fs_1.default.lstatSync(dir).isDirectory()) {
            files = await fast_glob_1.default([`${dir}/**/*`], { dot: false });
            isFile = false;
        }
        ...
        const txs = await deploy.prepare(dir, files, args.index, tags, ipfsPublish, license, useBundler, feeMultiplier, forceRedeploy, colors);
        const balAfter = await showDeployDetails_1.showDeployDetails(txs, wallet, isFile, dir, blockweave, useBundler, deploy.getBundler(), license, bundler, {
            tx: deploy.getBundledTx(),
            bundle: deploy.getBundle(),
        }, colors);
        if (balAfter < 0) {
            console.log(useBundler
                ? utils_1.parseColor(colors, "You don't have enough bundler balance for this deploy.", 'red')
                : utils_1.parseColor(colors, "You don't have enough balance for this deploy.", 'red'));
            return;
        }
        ...
        if (!res.confirm) {
            console.log(utils_1.parseColor(colors, 'Rejected!', 'red'));
            return;
        }
        if (ipfsPublish) {
            ...
            console.log('');
            console.log(utils_1.parseColor(colors, 'IPFS deployed! Main CID:', 'green'));
            console.log(utils_1.parseColor(colors, ipfsHash.cid.toString(), 'cyan'));
        }
        const manifestTx = await deploy.deploy(isFile, useBundler, colors);
        console.log('');
        if (useBundler) {
            console.log(utils_1.parseColor(colors, 'Data items deployed! Visit the following URL to see your deployed content:', 'green'));
        }
        else {
            console.log(utils_1.parseColor(colors, 'Files deployed! Visit the following URL to see your deployed content:', 'green'));
        }
        console.log(utils_1.parseColor(colors, `${blockweave.config.url}/${manifestTx}`, 'cyan'));
    },
};
